...
with a given string length-_n_, calculating the maximal size of intersection between two _l_-balls with radius 1, which ``x≠y`` is ``max|Bl(x;1)∩Bl(y;1)|`` is denoted by ``x`` and ``y`` which ``dl(x,y)=1``.
